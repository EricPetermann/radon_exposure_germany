...
Here in this app the model used in our paper (...) can be tested. 
    ...
    fluidRow(
      column(4,div(p(strong("Indoor radon prediction"),style="font-size:50px;color:#3474A7",class = "verticalcenter")))  #vertical alignment of title??
                ...
                fluidRow(
                   ...
                   tags$h3("1) Select a location  (within Germany)", style = "font-size:20px;color:#3474A7"),
                   textInput(inputId = "addresse", label = "Enter city or address", value = ""),
...
server <- function(input, output,session) {
  ...
  newdata <- eventReactive(input$Gebaeude_button,{
      ...
      colnames(newdata) <- x
      newdata[,1] <- as.factor(input$type)
      newdata[,1] <- mapvalues(newdata[,1], 
                                 from = c("Single family house","Townhouse","Multi family house",
                                          "Apartment block","High-rise apartment block","Terrace house","Farmhouse","Office building"), 
                                 to = c("EFH/ZFH", "RH/DHH","MFH","WB","WHH","TH","BH","Büro"))
      newdata[,2] <- as.factor(input$baujahr)
      newdata[,2] <- mapvalues(newdata[,2], 
                               from = c("Before 1945","1945 - 1980","1981 - 1995",
                                        "1996 - 2005","2006 or later","unknown"), 
                               to = c("<1945", "1945-1980","1981-1995","1996-2005",">2006","NA"))
      
      newdata[,3] <- as.factor(input$wohneinheiten)
      newdata[,3] <- mapvalues(newdata[,3], 
      ...
      newdata[,4] <- as.factor(input$etage)
      newdata[,4] <- mapvalues(newdata[,4], 
  ...
  observeEvent(input$Gebaeude_button, {
    shinyjs::enable("ML_button")
    shinyjs::toggle("ML_button", anim = TRUE)
  })
  ...
  data.for.pred <-  eventReactive(input$ML_button,{
    req(is.na(check()$water)) 
    req(is.na(check()$mining))  
    req(!is.na(check()$border))
    data<-newdata()
    data
  })
  
  output$pred_setting <- renderText({
    req(is.na(check()$water)) 
    req(is.na(check()$mining))  
    req(!is.na(check()$border))
    req(probs())
  ...
  probs <- eventReactive(input$ML_button,{
    ...
    req(is.na(check()$mining))  
    req(!is.na(check()$border))
    probs<-(1-plnorm3(q=c(100,300),mean=dist.par()$GM,sd=dist.par()$GSD,threshold=dist.par()$thresh))*100  
    P100<-probs[1]
    P300<-probs[2]
    probs<-data.frame(P100,P300)
    probs
  })
  ...
  prob100<-eventReactive(input$ML_button,{
    req(is.na(check()$water))
    req(is.na(check()$mining))
    req(!is.na(check()$border))
    ifelse(probs()$P100 <=1,paste0("The best estimate (conditional mean) for your household is an indoor radon concentration of ", strong(round(dist.par()$AM,digits=0)," Bq/m³."),
           "In 10 out of 100 cases (90th percentile) for households with similar environmental and dwelling characteristics, indoor radon concentrations are greater than ", round(dist.par()$P90,digits=0)," Bq/m³.",
          paste0("The probability of "),strong("exceeding 100 Bq/m³"), " for your household is ",strong("<1 %.")),
          paste0("The best estimate (conditional mean) for your household is an indoor radon concentration of ", strong(round(dist.par()$AM,digits=0)," Bq/m³."),
                  "In 10 out of 100 cases (90th percentile) for households with similar environmental and dwelling characteristics, indoor radon concentrations are greater than ", round(dist.par()$P90,digits=0)," Bq/m³.",
